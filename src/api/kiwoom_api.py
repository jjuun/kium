...
class KiwoomAPI:
    ...
    def _load_app_secret(self):
        ...
        try:
            ...
            logger.warning("앱 시크릿 파일을 찾을 수 없습니다: config/64339425_secretkey.txt")
            return None
        except Exception as e:
    ...
    def get_access_token(self):
        ...
        try:
            ...
            if not self.app_key or not self.app_secret:
                logger.error("API 키가 설정되지 않았습니다. app_key와 app_secret를 확인해주세요.")
                return None
    ...
    def place_order(self, symbol, quantity, price, order_type='01', price_type='00'):
        ...
        try:
            ...
            # 종목코드 정규화 (A 접두사 제거)
            normalized_symbol = symbol.replace('A', '') if symbol.startswith('A') else symbol
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'output' in result:
                    ...
                    logger.info(f"output 키들: {list(output.keys()) if isinstance(output, dict) else 'N/A'}")
                ...
                if success:
                    ...
                    if oso_data:
                        logger.info(f"미체결 주문 상세: {json.dumps(oso_data, indent=2, ensure_ascii=False)}")
                    
                    return result
                else:
                    error_msg = result.get('msg1', result.get('message', '알 수 없는 오류'))
                    logger.error(f"❌ 키움 API 미체결 주문 조회 실패 (ka10075): {error_msg}")
                    logger.error(f"실패 응답: {json.dumps(result, indent=2, ensure_ascii=False)}")
                    return result
            else:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"현재가 조회 500 에러 (시도 {attempt + 1}/{max_retries}): {symbol} - {response.text}")
                        ...
                        logger.error(f"현재가 조회 실패: {symbol} - {response.status_code} - {response.text}")
                    ...
                    logger.warning(f"현재가 조회 타임아웃 (시도 {attempt + 1}/{max_retries}): {symbol}")
                    if attempt < max_retries - 1:
                        time.sleep(1)
                        continue
                    else:
    ...
    def get_account_balance_kt00018(self, qry_tp: str = "1", dmst_stex_tp: str = "KRX",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"계좌평가잔고내역 조회 실패: {response.status_code} - {response.text}")
                # 에러가 발생해도 기본 응답 반환
                return {
                    'output': [],
                    'total_count': 0,
    ...
    def get_realized_pnl(self, stk_cd, strt_dt=None, cont_yn='N', next_key=''):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"실현손익 조회 실패: {response.status_code} - {response.text}")
                # 에러가 발생해도 기본 응답 반환
                return {
                    'output': [],
                    'total_count': 0,
    ...
    def get_execution_history(self, stk_cd: str = "A005935", qry_tp: str = "1", 
                            sell_tp: str = "0", ord_no: str = "", stex_tp: str = "0",
                            cont_yn: str = "N", next_key: str = "") -> Dict[str, Any]:
        """
        체결 내역 조회 (ka10076 TR)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"체결 내역 조회 실패: {response.status_code} - {response.text}")
                return {
                    'output': [],
                    'total_count': 0,
                    'error': f'API 오류: {response.status_code}',
    ...
    def get_execution_history_by_date(self, stk_cd: str = "A005935", 
                                    strt_dt: str = None, end_dt: str = None) -> Dict[str, Any]:
        """
    ...
    def get_stock_basic_info(self, stk_cd: str = "A005935", cont_yn: str = "N", next_key: str = "") -> Dict[str, Any]:
        """
        주식기본정보요청 (ka10001 TR)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"주식 기본정보 조회 실패: {response.status_code} - {response.text}")
                return None
    ...
    def validate_stock_code(self, stk_cd: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not (clean_stk_cd.isdigit() and len(clean_stk_cd) == 6) and \
               not (clean_stk_cd.startswith('A') and clean_stk_cd[1:].isdigit() and len(clean_stk_cd) == 7):
                return {
                    'valid': False,
                    'symbol': clean_stk_cd,
                    'name': '',
            ...
            if stock_info:
                ...
                if 'stk_nm' in stock_info:
                    ...
                    if isinstance(output, dict):
                        stock_name = output.get('hts_kor_isnm', '') or output.get('stk_nm', '')
                    elif isinstance(output, list) and len(output) > 0:
                        stock_name = output[0].get('hts_kor_isnm', '') or output[0].get('stk_nm', '')
            if stock_name:
                return {
                    'valid': True,
                    'symbol': clean_stk_cd,
    ...
    def get_positions(self):
        ...
        try:
            ...
            if balance_data:
                logger.info(f"계좌 잔고 응답 키: {list(balance_data.keys()) if isinstance(balance_data, dict) else 'Not a dict'}")
                if isinstance(balance_data, dict) and 'output' in balance_data:
                    ...
                    if isinstance(output_data, list):
                        ...
                        if len(output_data) > 0:
                            logger.info(f"첫 번째 항목 키: {list(output_data[0].keys()) if isinstance(output_data[0], dict) else 'Not a dict'}")
                ...
                if isinstance(output_data, list):
                        ...
                        if isinstance(item, dict):
                            ...
                            if hldg_qty and int(hldg_qty) > 0:
                                positions.append({
                                    ...
                                    'unrealized_pnl_rate': float(item.get('evlu_pfls_rt', 0))
                                })
                elif isinstance(output_data, dict):
                    # 단일 항목인 경우
                    ...
                    if hldg_qty and int(hldg_qty) > 0:
                        positions.append({
                            ...
                            'unrealized_pnl': float(output_data.get('evlu_pfls_amt', 0)),
                            'unrealized_pnl_rate': float(output_data.get('evlu_pfls_rt', 0))
                        })
            ...
            # 3. 단일 딕셔너리인 경우
                ...
                if hldg_qty and int(hldg_qty) > 0:
                    positions.append({
                        ...
                        'unrealized_pnl': float(balance_data.get('evlu_pfls_amt', 0)),
                        'unrealized_pnl_rate': float(balance_data.get('evlu_pfls_rt', 0))
                    })
        ...
        try:
            ...
            for item in self.watchlist:
                try:
                    ...
                    logger.warning(f"현재가 조회 실패 ({item['stk_cd']}): {price_error}")
                    item['current_price'] = 0
                    item['price_change'] = 0
                    item['price_change_rate'] = 0
    ...
    def add_watchlist(self, stk_cd: str):
        ...
        try:
            # 종목코드 정규화 (A 접두사 제거)
            normalized_stk_cd = stk_cd.replace('A', '') if stk_cd.startswith('A') else stk_cd
            ...
            if basic_info and 'output' in basic_info and basic_info['output']:
                ...
                logger.info(f"관심종목 등록 성공: {normalized_stk_cd} ({watchlist_item['stk_nm']})")
                ...
                logger.warning(f"종목 기본정보 조회 실패, 수동 등록 허용: {normalized_stk_cd}")
        ...
        try:
            ...
            for i, item in enumerate(self.watchlist):
                if item.get('stk_cd') == normalized_stk_cd:
                    ...
                    logger.info(f"관심종목 삭제 성공: {normalized_stk_cd} ({removed_item.get('stk_nm', normalized_stk_cd)})")
                    return {
                        'success': True,
    ...
    def get_stock_chart(self, stk_cd: str, tic_scope: str = '1', upd_stkpc_tp: str = '1', 
                       cont_yn: str = 'N', next_key: str = '') -> Dict[str, Any]:
        """
        주식분봉차트조회요청 (ka10080)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"주식분봉차트 조회 실패: {response.status_code} - {response.text}")
                return None
    ...
    def get_execution_balance_kt00005(self, dmst_stex_tp: str = "KRX", cont_yn: str = "N", next_key: str = "") -> Dict[str, Any]:
        """
        체결잔고요청 (kt00005)
