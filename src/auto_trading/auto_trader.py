...
class AutoTrader:
            ...
            for condition in conditions:
                ...
                if signal:
                    ...
                    signal_id = self.signal_monitor.record_signal(
                        ...
                        signal.condition_value, signal.current_price, signal.rsi_value
                    )
                    signal.id = signal_id
    ...
    async def _get_current_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            if not self.kiwoom_api.refresh_token_if_needed():
                ...
                logger.error("⚠️ 토큰 문제로 인한 자동 매매 차단 - 웹 대시보드에서 토큰을 확인하세요")
                return None
    ...
    async def _evaluate_condition(self, condition, current_price: float) -> Optional[TradingSignal]:
        ...
        try:
            ...
            elif condition.category == "price" or "<" in condition_value or ">" in condition_value:
                signal = self._evaluate_price_condition(condition, condition_value, current_price)
                if signal:
                    logger.info(f"가격 매매 신호 생성: {signal.symbol} {signal.signal_type} - {condition_value}")
                    return signal
    ...
    def _evaluate_price_condition(self, condition, condition_value: str, current_price: float) -> Optional[TradingSignal]:
        ...
        try:
            ...
            if "<" in condition_value:
                ...
                elif current_price > target_price and condition.condition_type == "sell":
                    return TradingSignal(
                        symbol=condition.symbol,
                        signal_type="sell",
                        condition_id=condition.id,
        ...
        try:
            ...
            if success:
                ...
                if hasattr(signal, 'id') and signal.id:
                    self.signal_monitor.update_signal_execution(signal.id, signal.current_price, self.trade_quantity)
                
                logger.info(f"매매 신호 실행 완료 ({mode_text}): {signal.symbol} {signal.signal_type}")
            else:
    ...
    def _check_risk_management(self, signal: TradingSignal) -> bool:
        ...
        try:
            ...
            if current_count >= max_count:
                logger.warning(f"일일 주문 수 제한 도달 ({mode_text}): {current_count}/{max_count}")
                return False
    ...
    async def _execute_buy_order(self, signal: TradingSignal) -> bool:
        ...
        try:
            if self.test_mode:
                ...
                logger.info(f"🔵 [테스트 모드] 매수 주문 실행: {signal.symbol} - {signal.condition_value} (수량: {self.trade_quantity})")
                logger.info(f"✅ 매수 주문 성공 (테스트): {signal.symbol} {self.trade_quantity}주")
                return True
            else:
                # 실제 주문 실행
                logger.info(f"🔵 [실제 매매] 매수 주문 실행: {signal.symbol} - {signal.condition_value} (수량: {self.trade_quantity})")
                
                # 액세스 토큰 확인 (실제 주문 실행 전 필수)
                if not self.kiwoom_api.access_token:
                    logger.error(f"❌ 매수 주문 실패 (실제): {signal.symbol} - 액세스 토큰이 없습니다")
                    return False
                
                # 신호에서 현재가 사용 (이미 조회됨)
                current_price = signal.current_price
                logger.info(f"📊 신호에서 현재가 사용: {signal.symbol} = {current_price}")
                
                if current_price is None or current_price <= 0:
                    logger.error(f"❌ 매수 주문 실패 (실제): {signal.symbol} - 현재가 조회 실패 (가격: {current_price})")
                    return False
                
                logger.info(f"💰 매수 주문 생성: {signal.symbol} {self.trade_quantity}주 @ {int(current_price)}원")
                ...
                logger.info(f"📋 주문 요청 생성 완료: {order_request.symbol} {order_request.order_type.value} {order_request.quantity}주 @ {int(order_request.price)}원")
                result = self.order_executor.place_order(order_request)
                
                if result and result.status.value in ['접수완료', 'ACCEPTED', '전체체결', 'FILLED']:
                    logger.info(f"✅ 매수 주문 성공 (실제): {signal.symbol} {self.trade_quantity}주 @ {int(current_price)}원")
                    logger.info(f"주문 상태: {result.status.value}, 주문 ID: {result.order_id}")
                    return True
                else:
    ...
    async def _execute_sell_order(self, signal: TradingSignal) -> bool:
        ...
        try:
            if self.test_mode:
                # 테스트 모드: 실제 주문 없이 성공으로 처리
                logger.info(f"🔴 [테스트 모드] 매도 주문 실행: {signal.symbol} - {signal.condition_value} (수량: {self.trade_quantity})")
                logger.info(f"✅ 매도 주문 성공 (테스트): {signal.symbol} {self.trade_quantity}주")
                return True
            else:
                # 실제 주문 실행
                logger.info(f"🔴 [실제 매매] 매도 주문 실행: {signal.symbol} - {signal.condition_value} (수량: {self.trade_quantity})")
                
                # 액세스 토큰 확인 (실제 주문 실행 전 필수)
                if not self.kiwoom_api.access_token:
                    logger.error(f"❌ 매도 주문 실패 (실제): {signal.symbol} - 액세스 토큰이 없습니다")
                    return False
                ...
                if current_price is None or current_price <= 0:
                    logger.error(f"❌ 매도 주문 실패 (실제): {signal.symbol} - 현재가 조회 실패 (가격: {current_price})")
                    return False
                
                logger.info(f"💰 매도 주문 생성: {signal.symbol} {self.trade_quantity}주 @ {int(current_price)}원")
                ...
                if result and result.status.value in ['접수완료', 'ACCEPTED', '전체체결', 'FILLED']:
                    logger.info(f"✅ 매도 주문 성공 (실제): {signal.symbol} {self.trade_quantity}주 @ {int(current_price)}원")
                    logger.info(f"주문 상태: {result.status.value}, 주문 ID: {result.order_id}")
                    return True
                else:
    ...
    def _reset_daily_order_count(self):
        ...
        if self.last_order_reset != current_date:
            ...
            logger.info(f"일일 주문 수 자동 리셋됨 (날짜 변경: {self.last_order_reset} → {current_date})")
    ...
    def get_last_error(self) -> Optional[Dict[str, Any]]:
        ...
        if self.last_error and self.error_timestamp:
            return {
                ...
                'age_minutes': int((datetime.now() - self.error_timestamp).total_seconds() / 60)
            }
        return None 