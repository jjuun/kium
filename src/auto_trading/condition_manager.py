...
class ConditionManager:
    ...
    def add_condition(
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(
                    f"조건 추가 완료: {symbol} {condition_type} {category} {value}"
                )
                return True
        except Exception as e:
    ...
    def get_conditions(
        self, symbol: str = None, condition_type: str = None, active_only: bool = False
    ) -> List[ConditionItem]:
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
    ...
    def get_condition(self, condition_id: int) -> Optional[ConditionItem]:
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id, symbol, condition_type, category, value, description, success_rate, total_signals, successful_signals, avg_profit, is_active, created_at, updated_at FROM conditions WHERE id = ?",
                    (condition_id,),
                )
                row = cursor.fetchone()
                if row:
    ...
    def update_condition_performance(
        self,
        condition_id: int,
        success_rate: float,
        total_signals: int,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(
                        f"조건 성과 업데이트 완료: {condition_id} - 성공률: {success_rate}%"
                    )
                    return True
                else:
    ...
    def create_condition_group(
        self, symbol: str, name: str, logic: str, priority: int = 5
    ) -> Optional[int]:
        """조건 그룹 생성"""
        try:
            with sqlite3.connect(self.db_path) as conn:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(
                        f"조건을 그룹에서 제거 완료: {group_id} -> {condition_id}"
                    )
                    return True
                else:
                    logger.warning(
                        f"조건을 그룹에서 제거 실패: {group_id} -> {condition_id}"
                    )
                    return False
        except Exception as e:
    ...
    def get_condition_groups(
        self, symbol: str = None, active_only: bool = False
    ) -> List[ConditionGroup]:
        """조건 그룹 목록 조회"""
        try:
            with sqlite3.connect(self.db_path) as conn:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(
                        f"그룹 우선순위 업데이트 완료: {group_id} -> {priority}"
                    )
                    return True
                else:
