...
class SignalMonitor:
    ...
    def update_signal_execution(
        self, signal_id: int, executed_price: float, executed_quantity: int
    ) -> bool:
        """
        신호 실행 정보 업데이트
    ...
    def close_signal(self, signal_id: int, profit_loss: float) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                status = (
                    SignalStatus.SUCCESS.value
                    if profit_loss > 0
                    else SignalStatus.FAILED.value
                )
    ...
    def get_signals(
        self, symbol: str = None, status: SignalStatus = None, days: int = 30
    ) -> List[SignalRecord]:
        """
        신호 목록 조회
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """.format(
                    ...
                    (SignalStatus.SUCCESS.value, SignalStatus.FAILED.value),
                )
                ...
                # 성공률 계산
                success_rate = (
                    (successful_signals / executed_signals * 100)
                ...
                return {
                    ...
                    "top_symbols": [
                        {"symbol": s[0], "count": s[1]} for s in top_symbols
                    ],
                    "days": days,
                }
