...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        try:
            ...
            # 기술적 지표 계산
            data_with_indicators = self.data_collector.calculate_technical_indicators(
                data
            )
        ...
        try:
            ...
            if action == "BUY":
                ...
                if self.current_capital >= total_cost:
                    ...
                    logger.info(
                        f"매수 실행: {signal['symbol']} {quantity}주 @ {price:,}원"
                    )
                ...
                if signal["symbol"] in self.positions:
                    ...
                    logger.info(
                        f"매도 실행: {signal['symbol']} {quantity}주 @ {price:,}원 (손익: {net_profit:,.0f}원)"
                    )
    ...
    def _update_positions(self, current_row: pd.Series, timestamp: datetime):
        ...
        try:
            ...
            for symbol, position in list(self.positions.items()):
                # 손절 확인
                stop_loss_price = position.avg_price * (
                ...
                # 익절 확인
                take_profit_price = position.avg_price * (
            ...
            self.equity_curve.append(
                {
                    ...
                    "return": (total_equity - self.initial_capital)
    ...
    def _calculate_results(self, data: pd.DataFrame) -> BacktestResult:
        ...
        try:
            ...
            if len(equity_df) > 1:
                ...
                if days > 0:
                    annualized_return = (
                        (final_equity / self.initial_capital) ** (365 / days) - 1
                    ) * 100
                else:
            ...
            if len(equity_df) > 0:
                ...
                equity_df["drawdown"] = (
                    (equity_df["total_equity"] - equity_df["cummax"])
                    / equity_df["cummax"]
                    * 100
                )
            ...
            # 수익 팩터 계산
            gross_profit = sum(
                [t.price * t.quantity for t in self.trades if t.action == "SELL"]
            )
            gross_loss = sum(
                [t.price * t.quantity for t in self.trades if t.action == "BUY"]
            )
