...
class BacktestRunner:
    ...
    def _save_optimization_results(
        self, results: pd.DataFrame, symbol: str, optimization_type: str
    ):
        """
        최적화 결과 저장
        """
    ...
    def generate_report(
        self, result: BacktestResult, symbol: str, output_dir: str = "reports"
    ):
        """
        백테스트 보고서 생성
        """
        try:
            ...
            analyzer.plot_monthly_returns(
                str(charts_dir / f"{symbol}_monthly_returns.png")
            )
            analyzer.plot_trade_analysis(
                str(charts_dir / f"{symbol}_trade_analysis.png")
            )
...
def main():
    ...
    parser.add_argument(
        "--start_date", type=str, required=True, help="시작 날짜 (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end_date", type=str, required=True, help="종료 날짜 (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--initial_capital", type=float, default=10000000, help="초기 자본금"
    )
    parser.add_argument(
        "--mode",
        type=str,
        choices=["single", "optimization"],
        default="single",
        help="실행 모드",
    )
    parser.add_argument(
        "--optimization_type",
        type=str,
        choices=["sma_crossover", "risk_parameters", "grid_search"],
        help="최적화 타입",
    )
