...
class StrategyOptimizer:
    def __init__(
        self, data: pd.DataFrame, symbol: str, initial_capital: float = 10000000
    ):
        """
        전략 최적화기 초기화
    ...
    def optimize_sma_crossover(
        ...
        try:
            ...
            param_combinations = list(
                product(short_periods, long_periods, stop_loss_range, take_profit_range)
            )

            # 유효한 조합만 필터링 (단기 < 장기)
            valid_combinations = [
                (s, l, sl, tp) for s, l, sl, tp in param_combinations if s < l
            ]

            logger.info(
                f"총 {len(valid_combinations)}개의 파라미터 조합으로 최적화 진행"
            )
                ...
                for future in as_completed(future_to_params):
                    ...
                    try:
                        ...
                        if result:
                            ...
                            logger.info(
                                f"완료: {params} -> 수익률: {result['total_return']:.2f}%"
                            )
            ...
            if not results_df.empty:
                ...
                logger.info(
                    f"최적화 완료: 최고 수익률 {results_df['total_return'].iloc[0]:.2f}%"
                )
    ...
    def optimize_risk_parameters(
        ...
        try:
            ...
            # 파라미터 조합 생성
            param_combinations = list(
                product(position_size_range, commission_range, slippage_range)
            )
            ...
            if not results_df.empty:
                ...
                logger.info(
                    f"리스크 최적화 완료: 최고 샤프 비율 {results_df['sharpe_ratio'].iloc[0]:.3f}"
                )
    ...
    def grid_search(
        ...
        try:
            ...
            logger.info(
                f"총 {len(param_combinations)}개의 파라미터 조합으로 최적화 진행"
            )
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_params = {
                    executor.submit(
                        self._run_grid_backtest, params, param_names
                    ): params
                    for params in param_combinations
                }
                ...
                for future in as_completed(future_to_params):
                    ...
                    try:
                        ...
                        if result:
                            results.append(result)
                            logger.info(
                                f"완료: {dict(zip(param_names, params))} -> {metric}: {result[metric]:.2f}"
                            )
                    except Exception as e:
                        logger.error(f"파라미터 {params} 백테스트 실패: {e}")
            ...
            if not results_df.empty:
                ...
                logger.info(
                    f"그리드 서치 완료: 최고 {metric} {best_params[metric]:.2f}"
                )
    ...
    def _run_grid_backtest(
        self, params: Tuple, param_names: List[str]
    ) -> Dict[str, Any]:
        """
        그리드 서치용 백테스트 실행
        """
    ...
    def plot_optimization_results(
        self, results_df: pd.DataFrame, metric: str = "total_return"
    ):
        """
        최적화 결과 시각화
        """
        try:
            ...
            plt.ylabel(metric)
            plt.xticks(
                range(len(top_results)), [f"#{i+1}" for i in range(len(top_results))]
            )
