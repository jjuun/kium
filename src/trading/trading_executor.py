    ...
    def place_buy_order(self, symbol, quantity, price=None):
        ...
        try:
            if self.is_simulation:
                ...
                logger.info(
                    f"시뮬레이션 매수 주문: {symbol} - {quantity}주 @ {price:,}원"
                )
                return order_result
            else:
                # 키움증권 API를 통한 실제 주문
                result = self.kiwoom_api.place_order(
                    symbol, quantity, price, "00"
                )  # 지정가 주문
                if result:
                    return {
                        "order_id": result.get(
                            "ODNO", f"KIWOOM_BUY_{int(time.time())}"
                        ),
                        "symbol": symbol,
                        "action": "BUY",
                        "quantity": quantity,
    ...
    def place_sell_order(self, symbol, quantity, price=None):
        ...
        try:
            if self.is_simulation:
                ...
                logger.info(
                    f"시뮬레이션 매도 주문: {symbol} - {quantity}주 @ {price:,}원"
                )
                return order_result
            else:
                # 키움증권 API를 통한 실제 주문
                ...
                if result:
                    return {
                        "order_id": result.get(
                            "ODNO", f"KIWOOM_SELL_{int(time.time())}"
                        ),
                        "symbol": symbol,
                        "action": "SELL",
                        "quantity": quantity,
            ...
            if self.is_simulation:
                ...
                if result:
                    return {
                        ...
                        "remaining_quantity": result.get("ORD_QTY", 0)
                        - result.get("EXEC_QTY", 0),
        ...
        try:
            if self.is_simulation:
                ...
                if result:
                    ...
                    return {
                        "cash": int(
                            balance_data.get("prvs_rcdl_excc_amt", 0)
                        ),  # 예수금
                        "total_value": int(
                            balance_data.get("tot_evlu_amt", 0)
                        ),  # 총평가금액
                        "buying_power": int(
                            balance_data.get("ord_psbl_cash", 0)
                        ),  # 주문가능현금
                        "timestamp": datetime.now(),
                    }
                return None
