...
class TradingStrategy:
    ...
    def sma_crossover_strategy(self, data):
        ...
        try:
            ...
            buy_signal = (
                previous['SMA_5'] <= previous['SMA_20'] and
            ...
            # 볼린저 밴드 위치 확인
            bb_position = (current['종가'] - current['BB_Lower']) / (current['BB_Upper'] - current['BB_Lower'])
            ...
            if signal['action'] != 'HOLD':
                logger.info(f"매매 신호: {signal['action']} - 신뢰도: {signal['confidence']:.2f} - 이유: {', '.join(signal['reason'])}")
    ...
    def combined_strategy(self, data):
        ...
        try:
            ...
            if final_signal['action'] != 'HOLD':
                logger.info(f"복합 전략 신호: {final_signal['action']} - 신뢰도: {final_signal['confidence']:.2f}")
