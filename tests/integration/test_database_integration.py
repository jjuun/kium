...
class TestDatabaseIntegration:
    ...
    def test_database_initialization(self, temp_db_path):
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='trading_conditions'"
        )
        assert cursor.fetchone() is not None
    ...
    def test_trading_conditions_persistence(self, auto_trader_with_db):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id FROM trading_conditions WHERE id = ?", (condition_id,)
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def test_multiple_conditions_management(self, auto_trader_with_db):
        ...
        # 특정 종목의 조건만 조회
        symbol_conditions = (
            auto_trader_with_db.condition_manager.get_conditions_by_symbol("005935")
        )
    ...
    def test_database_transaction_rollback(self, auto_trader_with_db):
        ...
        with patch.object(
            auto_trader_with_db.watchlist_manager, "_execute_query"
        ) as mock_execute:
    ...
    def test_database_migration_compatibility(self, temp_db_path):
        ...
        auto_trader_old.condition_manager.add_condition(
            "005935", "buy", "rsi", "RSI < 30"
        )
