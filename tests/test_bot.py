...
def test_data_collection():
    ...
    if historical_data is not None:
        ...
        print(
            f"   ✓ 데이터 범위: {historical_data.index[0]} ~ {historical_data.index[-1]}"
        )
        ...
        if data_with_indicators is not None:
            ...
            print(
                f"   ✓ 단기 이동평균: {data_with_indicators['SMA_5'].iloc[-1]:,.0f}원"
            )
            print(
                f"   ✓ 장기 이동평균: {data_with_indicators['SMA_20'].iloc[-1]:,.0f}원"
            )
...
def test_risk_management():
    ...
    quantity, position_size = risk_manager.calculate_position_size(
        current_price, available_capital
    )
    ...
    # 손절 테스트 (가격 하락)
    stop_loss_triggered, stop_loss_info = risk_manager.check_stop_loss(
        "005930", current_price * 0.95
    )
    print(f"   ✓ 손절 조건 확인: {stop_loss_triggered}")

    # 익절 테스트 (가격 상승)
    take_profit_triggered, take_profit_info = risk_manager.check_take_profit(
        "005930", current_price * 1.06
    )
