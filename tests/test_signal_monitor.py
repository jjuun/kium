...
class TestSignalMonitor(unittest.TestCase):
    ...
    def test_update_signal_execution(self):
        ...
        success = self.signal_monitor.update_signal_execution(
            signal_id=signal_id, executed_price=70500.0, executed_quantity=10
        )
    ...
    def test_close_signal(self):
        ...
        self.signal_monitor.update_signal_execution(
            signal_id=signal_id2, executed_price=99000.0, executed_quantity=5
        )
    ...
    def test_get_signal_statistics(self):
        """신호 통계 조회 테스트"""
        # 여러 신호 기록 (성공/실패 포함)
        signal_id1 = self.signal_monitor.record_signal(
            "005930", "buy", 1, "RSI < 30", 70000.0
        )
        signal_id2 = self.signal_monitor.record_signal(
            "000660", "sell", 2, "MA > 100000", 100000.0
        )
        signal_id3 = self.signal_monitor.record_signal(
            "005380", "buy", 3, "Volume > 1000000", 50000.0
        )
